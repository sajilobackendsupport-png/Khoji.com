...
export default function ProfileModal({
  ...
  const handleCreateCitizen = (e: React.FormEvent) => {
    e.preventDefault();
    if (!newCitizenName.trim() || !newCitizenPhone.trim()) {
      setCitizenError("Please enter full name and phone number.");
      return;
    }
        ...
        <div className="p-6 overflow-y-auto flex-1 space-y-4">
          ...
          {activeTab === "add-citizen" && (
            <form onSubmit={handleCreateCitizen} className="space-y-4">
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-slate-700">Nepalese Mobile Phone *</label>
                <input
                  type="tel"
                  required
